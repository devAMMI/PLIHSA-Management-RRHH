...
export function GoalDefinitionViewer({ definition, onClose, onUpdate, mode: initialMode = 'view' }: GoalDefinitionViewerProps) {
  ...
  const generatePdfBlob = async (): Promise<{ url: string; fileName: string } | null> => {
    try {
      ...
      doc.text('Desempeño Administrativo', pageWidth / 2, y + 11, { align: 'center' });

      doc.setFontSize(7);
      doc.setFont('helvetica', 'normal');
      doc.text('Código: PL-RH-P-002-F01', pageWidth - margin, y + 4, { align: 'right' });
      ...
      doc.setFontSize(9);
      doc.setFont('helvetica', 'bold');
      doc.text('DEFINICIÓN METAS INDIVIDUALES', margin + 2, y + 5);
      doc.setTextColor(0, 0, 0);
      y += 7;

      autoTable(doc, {
        head: [['No.', 'Metas Individuales', 'Medición y Resultados Esperados']],
        body: goals.map(g => [String(g.number), g.description || '-', g.measurement || '-']),
        startY: y,
      ...
      doc.setFontSize(9);
      doc.setFont('helvetica', 'bold');
      doc.text('DEFINICIÓN DE COMPETENCIAS CONDUCTUALES/HABILIDADES', margin + 2, y + 5);
      doc.setTextColor(0, 0, 0);
      y += 7;

      autoTable(doc, {
        head: [['No.', 'Conductas/Habilidades (Definir las 5 Principales)']],
        body: behaviors.map(b => [String(b.number), b.description || '-']),
        startY: y,
        margin: { left: margin, right: margin },
        styles: { fontSize: 8, cellPadding: 2, overflow: 'linebreak' as const },
        headStyles: { fillColor: [241, 245, 249], textColor: 0, fontStyle: 'bold' as const },
