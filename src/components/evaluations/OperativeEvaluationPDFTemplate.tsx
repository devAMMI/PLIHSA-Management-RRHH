    ...
    <div id={id} className="bg-white p-8 w-[210mm] mx-auto" style={{ fontFamily: 'Arial, sans-serif' }}>
      <div className="mb-6 border-b-2 border-blue-600 pb-4">
        ...
        <h1 className="text-xl font-bold text-center text-blue-900 mt-2">
          Evaluación de Desempeño - Personal Operativo
        </h1>
      ...
      <div className="mb-6">
        ...
        <div className="border border-slate-300 rounded-b-lg p-4 bg-slate-50">
          {functionalFactors.filter(f => f.job_function).map((factor) => (
            <div key={factor.factor_number} className="mb-4 pb-4 border-b border-slate-200 last:border-0">
              <div className="font-semibold text-sm mb-2">Factor {factor.factor_number}</div>
              <div className="text-sm mb-2">
                <span className="font-medium">Función del Puesto:</span> {factor.job_function}
              </div>
              <div className="text-sm">
                <span className="font-medium">Resultados Esperados:</span> {factor.expected_results}
              </div>
            </div>
          ))}
        </div>
