...
import { Save, FileText, Calendar, Download, Printer, Upload, CheckCircle, Eye, X, AlertCircle } from 'lucide-react';
...
export function AdministrativeEvaluationForm({ editingEvaluationId, onCancel, periodId }: AdministrativeEvaluationFormProps) {
    ...
    setTimeout(() => {
      ...
      if (printWindow && pdfEl) {
        printWindow.document.write(`
          <html>
            <head>
              <title>Evaluación Administrativa</title>
  ...
  const handleUploadSignedDocument = async () => {
    ...
    try {
      ...
      if (selectedEmployee?.employee_code && selectedEmployee?.last_name) {
        ...
        filePath = `PLIHSA/evaluacion/administrativo/${year}/${savedEvaluationId}.${fileExt}`;
      }
        ...
        <div className="mt-6 bg-white border border-slate-200 rounded-xl shadow-sm p-6">
          ...
          <div className="border-2 border-dashed border-slate-300 rounded-lg p-6 text-center hover:border-blue-400 transition mb-5">
            <input
              ...
              className="hidden"
            />
            <label htmlFor="signed-doc-upload" className="cursor-pointer flex flex-col items-center gap-3">
              {selectedFile ? (
                <>
